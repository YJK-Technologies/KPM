...
const VendorProductTable = () => {
  ...
  //       "Start Year": safeValue(formatDate(row.Start_Year)),
  //       "End Year": safeValue(formatDate(row.End_Year)),
  //       "Start No": safeValue(row.Start_No),
  //       "Running No": safeValue(row.Running_No),
  //       "End No": safeValue(row.End_No),
  //     };
  //   });

  //   const reportWindow = window.open("", "_blank");
  //   reportWindow.document.write("<html><head><title>Number Series</title>");
  ...
  //   reportWindow.document.write("<h1><u>Number Series Information</u></h1>");
...
const generateReport = () => {
  ...
  reportWindow.document.write(
    "<html><head><title>Number Series</title>"
  );
  ...
  reportWindow.document.write(
    "<h1><u>Number Series Information</u></h1>"
  );
  ...
  const deleteSelectedRows = async () => {
    ...
    const ScreenTypdeDelete = {
      Screen_TypesToDelete: selectedRows,
    };
      ...
      async () => {
        try {
          ...
          if (response.ok) {
            ...
            const errorResponse = await response.json();
            toast.warning(errorResponse.message || "Failed to delete rows");
          }
        } catch (error) {
    ...
    <div className="container-fluid ">
      ...
      <div className="card shadow-lg border-0 p-3 rounded-5 " style={{ height: "auto" }}>
        <div className="d-flex justify-content-between">
          <div className='d-flex justify-content-start'> <h4 className="mb-5 fw-semibold text-dark fs-2 fw-bold">Number Series</h4> </div>
          <div className="desktopbuttons">
            <div className='d-flex justify-content-end row'>
              {['add', 'all permission'].some(permission => numberSeriesPermissions.includes(permission)) && (
                <div className='col-md-2 mt-1 me-0 mb-5' ><a className='border-none text-dark p-1' title="Add Number Series" onClick={handleClick} style={{ cursor: "pointer" }}><svg xmlns="http://www.w3.org/2000/svg" width="23" height="23" fill="currentColor" class="bi bi-patch-plus-fill" viewBox="0 0 16 16">
                  <path d="M10.067.87a2.89 2.89 0 0 0-4.134 0l-.622.638-.89-.011a2.89 2.89 0 0 0-2.924 2.924l.01.89-.636.622a2.89 2.89 0 0 0 0 4.134l.637.622-.011.89a2.89 2.89 0 0 0 2.924 2.924l.89-.01.622.636a2.89 2.89 0 0 0 4.134 0l.622-.637.89.011a2.89 2.89 0 0 0 2.924-2.924l-.01-.89.636-.622a2.89 2.89 0 0 0 0-4.134l-.637-.622.011-.89a2.89 2.89 0 0 0-2.924-2.924l-.89.01zM8.5 6v1.5H10a.5.5 0 0 1 0 1H8.5V10a.5.5 0 0 1-1 0V8.5H6a.5.5 0 0 1 0-1h1.5V6a.5.5 0 0 1 1 0" />
                </svg>
                </a>
                </div>
              )}
              {['update', 'all permission'].some(permission => numberSeriesPermissions.includes(permission)) && (
              ...
              {['delete', 'all permission'].some(permission => numberSeriesPermissions.includes(permission)) && (
              ...
              {['view', 'all permission'].some(permission => numberSeriesPermissions.includes(permission)) && (
