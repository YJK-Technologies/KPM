...
const PurchaseDeletePopup = ({ open, handleClose, handlePurchaseDeleteData, apiPath }) => {
      ...
      const response = await fetch(`${config.apiBaseUrl}/getpurDeleteDetails`, {
        ...
        body: JSON.stringify({
           company_code: sessionStorage.getItem('selectedCompanyCode'),
            ...
            transaction_date, 
            vendor_code, 
            vendor_name, purchase_type:selectedPay, pay_type}) // Send company_no and company_name as search criteria
      });
      if (response.ok) {
        const searchData = await response.json();
        setRowData(searchData);
          ...
          <div className="modal-dialog modal-dialog-centered modal-lg mt-0" role="document">
            <div className="modal-content rounded-4 shadow-lg">
              <div className="modal-header">
                <h5 className="modal-title fw-bold fs-3">{apiPath === "deletedPurchaseOrderSearchData" ? "Deleted Purchase Order Help" : "Deleted Purchase Help"}</h5>
...
export default PurchaseDeletePopup;
